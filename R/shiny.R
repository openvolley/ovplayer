...
#' @param host string: shiny host. Default to '127.0.0.1' 
...
ovp_shiny <- function(playlist, video_server = NULL, launch_browser = TRUE, mobile = FALSE, host = "127.0.0.1",...) {
    ...
    if(mobile){
        this_app <- list(ui = ovp_shiny_mobile_ui(app_data = app_data), server = ovp_shiny_mobile_server(app_data = app_data))
    }
